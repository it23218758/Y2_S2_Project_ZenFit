...
export default function UpdateFeedback() {
  ...
  useEffect(() => {
    const fetchFeedback = async () => {
      try {
        const response = await fetch(`http://localhost:8070/feedback/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => navigate("/manage-feedbacks"), 2000); // Navigate after success
      } else {
        const data = await response.json();
        setSuccess("");
  ...
  return (
    <div className="container col-lg-6 my-5">
      <h3 className="mb-4 text-center">Update Feedback</h3>
