...
export default function UpdateTrainer() {
  ...
  useEffect(() => {
    ...
    if (decoded?.role === 'admin') {
      ...
      alert("Access denied. Only admins can update trainers.");
      navigate('/');
    }
  }, [navigate]);
  ...
  useEffect(() => {
    const fetchTrainer = async () => {
      try {
        const response = await fetch(`http://localhost:8070/trainer/${trainerId}`);
        if (response.ok) {
    ...
    if (validateForm()) {
      try {
        ...
        if (response.ok) {
          ...
          alert(errorData.error || 'Error occurred while updating trainer');
        }
      } catch (error) {
  ...
  return (
    <div className="d-flex justify-content-center align-items-center vh-100">
      <div className="card p-4 shadow col-lg-7">
        <h3 className="mb-4 text-center">Update Trainer</h3>
        <form noValidate onSubmit={handleSubmit}>
          ...
          <div className="text-center">
            <button className="btn btn-primary mt-2" type="submit">Update Trainer</button>
          </div>
        </form>
      </div>
    </div>
